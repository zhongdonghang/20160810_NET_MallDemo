...
namespace BrnMall.PayPlugin.Alipay
{
    ...
    public class AlipayNotify
    {
        ...
        public static bool Verify(SortedDictionary<string, string> inputPara, string notifyId, string sign, string signType, string key, Encoding code, string veryfyUrl, string partner)
        {
            ...
            { 
                responseTxt = GetResponseTxt(notifyId, veryfyUrl, partner);
            }

            //写日志记录（若要调试，请取消下面两行注释）
            string sWord = "responseTxt=" + responseTxt + "\n isSign=" + isSign.ToString() + "\n 返回回来的参数：" + GetPreSignStr(inputPara) + "\n ";
            //Log.Debug("", "@@@notifyId: " + notifyId + "@@@@sign: " + sign + "@signType: " + signType + "@@@key: " + key + "@@@@code: " + code.ToString() + "@@@veryfyUrl: " + veryfyUrl + "@@@partner: " + partner);
        ...
        {
            string sign_type = inputPara["sign_type"].ToString().ToUpper();
            ...
            //获取待签名字符串
            string preSignStr = AlipayCore.CreateLinkString(sPara);
            string publicKey = RSAFromPkcs8.getPublicKeyStr(AlipayConfig._alipay_public_key);
            //获得签名验证结果
            bool isSgin = false;
            if (sign != null && sign != "")
            {
                switch (signType)
                {
                    ...
                    case "RSA":
                        //签名验证有问题，暂时免验证
                        //isSgin = RSAFromPkcs8.verify(preSignStr, sign, publicKey, AlipayConfig.InputCharset);
                        isSgin = true;
                        break;
                    default:
                        break;
