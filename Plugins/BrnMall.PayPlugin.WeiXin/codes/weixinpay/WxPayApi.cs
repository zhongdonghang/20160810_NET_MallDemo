    ...
    public class WxPayApi
    {
        ...
        */
        public static WxPayData Micropay(WxPayData inputObj, int timeOut = 10)
        {
            ...
            inputObj.SetValue("spbill_create_ip", WxPayConfig._ip);//终端ip
        ...
        */
        public static WxPayData OrderQuery(WxPayData inputObj, string tradeType="", int timeOut = 6)
        {
            ...
            //ios、android和微信、网页是用不一样的商户号
            if (tradeType == "APP")
            {
                inputObj.SetValue("appid", WxPayConfig.IosAppId);//IOS公众账号ID
                inputObj.SetValue("mch_id", WxPayConfig.IosMchId);//IOS商户号
            }
            else if (tradeType == "ANDROID")
            {
                inputObj.SetValue("appid", WxPayConfig.AndroidAppId);//安卓公众账号ID
                inputObj.SetValue("mch_id", WxPayConfig.AndroidMchId);//安卓商户号
            }
            else
            {
                inputObj.SetValue("appid", WxPayConfig.AppId);//公众账号ID
        ...
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6)
        {
            ...
            {
                inputObj.SetValue("notify_url", BMAConfig.MallConfig.SiteUrl+WxPayConfig._notify_url);//异步通知url
            }
	    ...
	    */
        ...
        {
            //如果不需要进行上报
            if (WxPayConfig._report_levenl == 0)
            {
                return;
            }

            //如果仅失败上报
            if (WxPayConfig._report_levenl == 1 && inputObj.IsSet("return_code") && inputObj.GetValue("return_code").ToString() == "SUCCESS" &&
	    ...
	    */
        public static WxPayData Report(WxPayData inputObj, int timeOut = 1)
        {
            ...
            inputObj.SetValue("appid", WxPayConfig.AppId);//公众账号ID
            inputObj.SetValue("mch_id", WxPayConfig.MchId);//商户号
            inputObj.SetValue("user_ip", WxPayConfig._ip);//终端ip
