...
namespace BrnMall.Web.Controllers
{
    ...
    public partial class CatalogController : BaseWebController
    {
        ...
        public ActionResult Search()
        {
            ...
            if (string.IsNullOrWhiteSpace(keyword) && cateId > 0)
            {
                return Redirect(Url.Action("category", new RouteValueDictionary { { "cateId", cateId } }));
            }
            ...
            else//当搜索词为商品关键词时
            {
                ...
                {
                    categoryList = Categories.GetCategoryListByLayer(1);
                }
                else
                {
            ...
            //分页对象
            PageModel pageModel = new PageModel(PageSize, page, Searches.GetSearchMallProductCount(strkeyword, cateId, brandId, filterPrice, catePriceRangeList, null, onlyStock));
            //视图对象
            MallSearchModel model = new MallSearchModel()
            {
                ...
                AAndVList = null,
                PageModel = pageModel,
                ProductList = Searches.SearchMallProducts(pageModel.PageSize, pageModel.PageNumber, strkeyword, cateId, brandId, filterPrice, catePriceRangeList, null, onlyStock, sortColumn, sortDirection)
            };
        ...
        public ActionResult BrandList()
        {
            //分类id
            int cateId = GetRouteInt("cateId");
            if (cateId == 0 && WebHelper.GetQueryInt("cateId") == 0)
            {
                var lay1 = Categories.GetCategoryListByLayer(1);
                foreach (var info in lay1)
                {
                    cateId = info.CateId;
                    break;
                }
