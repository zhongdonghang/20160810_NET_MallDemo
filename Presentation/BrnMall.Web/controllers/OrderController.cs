...
namespace BrnMall.Web.Controllers
{
    ...
    public partial class OrderController : BaseWebController
    {
        ...
        public ActionResult ConfirmOrder()
        {
            ...
            //配送方式
            int shipmode = WebHelper.GetFormInt("shipmode");
            ...
            foreach (StoreCartInfo item in storeCartList)
            {
                ...
                storeOrder.ProductAmount = Carts.SumOrderProductAmount(item.SelectedOrderProductList, WorkContext.Uid);
                storeOrder.FullCut = Carts.SumFullCut(item.CartItemList);
                storeOrder.ShipFee = model.DefaultFullShipAddressInfo == null ? 0 : Orders.GetShipFee(model.DefaultFullShipAddressInfo.ProvinceId, model.DefaultFullShipAddressInfo.CityId, item.SelectedOrderProductList, storeOrder.ProductAmount, shipmode);
        ...
        public ActionResult GetValidCouponList()
        {
            ...
            foreach (DataRow row in Coupons.GetUnUsedCouponList(WorkContext.Uid).Rows)
            {
                ...
                else
                {
                    ...
                    if (TypeHelper.ObjectToInt(row["orderamountlower"]) > Carts.SumOrderProductAmount(storeCartInfo.SelectedOrderProductList, WorkContext.Uid))
                    {
                        break;
                    }
                    else
        ...
        public ActionResult SubmitOrder()
        {
            lock (_locker)
            {
                ...
                string[] buyerRemarks = buyerRemark.Split('&');
                int shipmode = WebHelper.GetFormInt("shipmode");
                //验证验证码
                ...
                //验证买家备注的内容长度
                for (int i = 0; i < buyerRemarks.Length - 1; i++)
                {
                    if (StringHelper.GetStringLength(buyerRemarks[i]) > 125)
                    {
                ...
                foreach (CouponInfo couponInfo in couponList)
                {
                    ...
                    else
                    {
                        ...
                        if (couponTypeInfo.OrderAmountLower > Carts.SumOrderProductAmount(storeCartInfo.SelectedOrderProductList, WorkContext.Uid))
                        {
                ...
                foreach (StoreCartInfo storeCartInfo in storeCartList)
                {
                    ...
                    OrderInfo orderInfo = Orders.CreateOrder(WorkContext.PartUserInfo, storeCartInfo.StoreInfo, storeCartInfo.SelectedOrderProductList, storeSinglePromotionList, fullShipAddressInfo, payPluginInfo, ref payCreditCount, storeCouponList, storeFullCut, buyerRemarks[rmark], bestTime, WorkContext.IP,shipmode);
                    rmark += 1;
                    if (orderInfo != null)
                    {
        ...
        public ActionResult SubmitResult()
        {
            ...
            List<OrderProductInfo> orderProList = new List<OrderProductInfo>();
            foreach (string oid in StringHelper.SplitString(oidList))
            {
                ...
                {
                    orderProList.AddRange(AdminOrders.GetOrderProductList(oid));
                }
                else
                {
            ...
            {
                return PromptView("订订单信息错误，请刷新。");
            }
            
            SubmitResultModel model = new SubmitResultModel();
            model.OidList = oidList;
            model.OrderProductList = orderProList;
            model.PayPluginList = Plugins.GetPayPluginList();
            if (paySystemName == "creditpay" && allPayMoney > WorkContext.UserAmount)
            {
                model.PayPlugin = Plugins.GetDefaultPayPlugin();
            }
            else
            {
        ...
        /// <returns></returns>
        public ActionResult ChangeOrderPlugin(string pluginName, string oids)
        {
            try
            {

            PluginInfo pinfo = Plugins.GetPayPluginBySystemName(pluginName);
            if (pinfo == null)
            {
                return AjaxResult("error","支付插件不存在，请刷新");
            }
            if(string.IsNullOrWhiteSpace(oids))
            {
            return AjaxResult("error","订单信息错误，请刷新");
            }
              foreach (string oidstr in StringHelper.SplitString(oids))
            {
                //订单信息
                int oid = TypeHelper.StringToInt(oidstr);
                ...
                Orders.UpdateOrderPayPlugin(oid, 1, pinfo.SystemName, pinfo.FriendlyName);
            }

              return AjaxResult("success", Url.Action("SubmitResult", new { oidList = oids }));
        ...
        public ActionResult PayShow()
        {
            ...
            {
                return RedirectToAction("NativePay", "WeiXin", new RouteValueDictionary { { "oidList", oidList } });
            }
            else
            {
                return RedirectToAction("Pay", "Alipay", new RouteValueDictionary { { "oidList", oidList } });
            }
        }
        ...
        public ActionResult PayResult()
        {
            ...
            foreach (string oid in StringHelper.SplitString(StringHelper.SubString(oidList, oidList.Length - 10),"A"))
            {
                //订单信息
                OrderInfo orderInfo = Orders.GetOrderByOid(TypeHelper.StringToInt(oid));
