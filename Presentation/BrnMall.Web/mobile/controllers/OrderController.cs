...
namespace BrnMall.Web.Mobile.Controllers
{
    ...
    public partial class OrderController : BaseMobileController
    {
        ...
        public ActionResult ConfirmOrder()
        {
            ...
            //默认支付插件设置为微信支付
            model.DefaultPayPluginInfo = Plugins.GetPayPluginBySystemName("weixin");
            if (payName.Length > 0)
            {
            ...
            {
                model.AllProductAmount += Carts.SumOrderProductAmount(item.SelectedOrderProductList,WorkContext.Uid);
                model.AllFullCut += Carts.SumFullCut(item.CartItemList);
                model.AllShipFee += model.DefaultFullShipAddressInfo != null ? Orders.GetShipFee(model.DefaultFullShipAddressInfo.ProvinceId, model.DefaultFullShipAddressInfo.CityId, item.SelectedOrderProductList, model.AllProductAmount,shipmode) : 0;
        ...
        public ActionResult SubmitOrder()
        {
            lock (_locker)
            {
                ...
                foreach (StoreCartInfo storeCartInfo in storeCartList)
                {
                    ...
                    orderInfo = Orders.CreateOrder(WorkContext.PartUserInfo, storeCartInfo.StoreInfo, storeCartInfo.SelectedOrderProductList, storeSinglePromotionList, fullShipAddressInfo, payPluginInfo, ref payCreditCount, storeCouponList, storeFullCut, "", new DateTime(1970, 1, 1), WorkContext.IP,shipmode);
                    if (orderInfo != null)
                    {
        ...
        public ActionResult SubmitResult()
        {
            ...
            model.OrderProList = orderProList;
            model.PayPluginList = Plugins.GetPayPluginList();
