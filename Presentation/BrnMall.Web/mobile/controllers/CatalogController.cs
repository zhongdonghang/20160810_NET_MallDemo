...
namespace BrnMall.Web.Mobile.Controllers
{
    ...
    public partial class CatalogController : BaseMobileController
    {
        ...
        public ActionResult Product()
        {
            ...
            //商品评价
            var reviewList = ProductReviews.GetProductReviewList(pid, 0, int.MaxValue, 1);
            model.ReviewpageModel = new PageModel(10, 1, reviewList.Rows.Count);
            model.ProductReviewList = ProductReviews.GetProductReviewList(pid, 0, model.ReviewpageModel.PageSize, 1);
        ...
        public ActionResult AjaxProductReviewList()
        {
            ...
            AjaxReviewModel model = new AjaxReviewModel();
            var reviewList = ProductReviews.GetProductReviewList(pid, 0, int.MaxValue, 1);
            model.ReviewpageModel = new PageModel(10, page, reviewList.Rows.Count);
            model.ProductReviewList = CommonHelper.DataTableToList(ProductReviews.GetProductReviewList(pid, 0, model.ReviewpageModel.PageSize, page));
        ...
        public ActionResult Category()
        {
            ...
            //分页对象
            PageModel pageModel = new PageModel(pageSize, page, Products.GetCategoryProductCount(cateId, brandId, filterPrice, catePriceRangeList, attrValueIdList, onlyStock));
            //视图对象
            CategoryModel model = new CategoryModel()
            {
                ...
                ChildCateList=Categories.GetChildCategoryList(cateId,2,false)
            };
        ...
        {
            //是否为搜索全部商品
            int isall = WebHelper.GetQueryInt("isall");
            //搜索词
            string keyword = WebHelper.GetQueryString("keyword").Trim();
            ...
            if (isall != 1)
            {
                ...
                //异步保存搜索历史
                //通过搜索框进行搜索（stype=1）的全部记录
                int type = GetRouteInt("stype");
                if (type == 0)
                {
                    type = WebHelper.GetQueryInt("stype");
                }
                if (type == 1 && !string.IsNullOrWhiteSpace(keyword))
                {
        ...
        public ActionResult AjaxSearch()
        {
            ...
            //价格范围列表
            string[] catePriceRangeList = StringHelper.SplitString("0-200,200-500,500-2000,2000-5000,5000", ",");
            ...
            //分页对象
            PageModel pageModel = new PageModel(pageSize, page, Searches.GetSearchMallProductCount(keyword, cateId, brandId, filterPrice, catePriceRangeList, null, onlyStock));
            //视图对象
            AjaxMallSearchModel model = new AjaxMallSearchModel()
            {
                PageModel = pageModel,
                ProductList = Searches.SearchMallProducts(pageModel.PageSize, pageModel.PageNumber, keyword, cateId, brandId, filterPrice, catePriceRangeList, null, onlyStock, sortColumn, sortDirection)
            };
