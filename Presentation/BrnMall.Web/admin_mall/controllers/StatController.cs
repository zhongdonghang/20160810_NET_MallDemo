...
namespace BrnMall.Web.MallAdmin.Controllers
{
    ...
    public partial class StatController : BaseMallAdminController
    {
        ...
        {
            int wordcount = AdminSearchHistories.GetSearchWordStatCount(word);
            PageModel pageModel = new PageModel(pageSize, pageNumber, wordcount);

            SearchWordStatListModel model = new SearchWordStatListModel()
            {
                SearchWordStatList = AdminSearchHistories.GetSearchWordStatList(pageModel.PageSize, pageModel.PageNumber, word, null),
                PageModel = pageModel,
                SortColumn = sortColumn,
                SortDirection = sortDirection,
                Word = word
        ...
        public ActionResult SaleTrend(string startTime = "0", string endTime = "23", int trendType = 0, int timeType = 0)
        {
            ...
            if (timeType == 3)//按年筛选
            {
                ...
                model.TimeType = 3;
                model.yunit = AdminOrders.GetMaxY(trendType, 3, startYear, endYear)/20;
            }
            else if (timeType == 2)//按月筛选
            {
                ...
                model.TimeType = 2;
                model.yunit = AdminOrders.GetMaxY(trendType, 2, startMonth, endMonth) / 20;
            }
            else if (timeType == 1)//按天筛选
            {
                ...
                model.TimeType = 1;
                model.yunit = AdminOrders.GetMaxY(trendType, 1, startDay, endDay) / 20;
            }
            else//按小时筛选
            {
                ...
                model.TimeType = 0;
                model.yunit = AdminOrders.GetMaxY(trendType, 0, startTime, endTime) / 20;
            }
            model.yunit = model.yunit == 0 ? 1 : model.yunit;
