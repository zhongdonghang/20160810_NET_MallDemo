...
namespace BrnMall.Web.MallAdmin.Controllers
{
    ...
    public partial class ProductController : BaseMallAdminController
    {
        ...
        {
            storeName=string.IsNullOrEmpty(storeName)?"": storeName.Trim();
            productName = string.IsNullOrEmpty(productName) ? "" : productName.Trim();
            categoryName = string.IsNullOrEmpty(categoryName) ? "" : categoryName.Trim();
            brandName = string.IsNullOrEmpty(brandName) ? "" : brandName.Trim();
        ...
        public ActionResult EditProduct(int pid = -1)
        {
            ...
            model.MemberPrice = productInfo.MemberPrice;
            model.VipPrice = productInfo.VipPrice;
            ...
            BrandInfo brand = AdminBrands.GetBrandById(productInfo.BrandId);
            model.BrandName = brand == null ? "选择品牌" : brand.Name;

            CategoryInfo cateInfo = AdminCategories.GetCategoryById(productInfo.CateId);
            model.CateName = cateInfo == null ? "选择分类" : cateInfo.Name;
        ...
        public ActionResult EditProduct(EditProductModel model, int pid = -1)
        {
            ...
            if (ModelState.IsValid)
            {
                if (model.CateId != productInfo.CateId || model.BrandId!=productInfo.BrandId)
                {
                    //删除旧商品类别关联的品牌缓存
                    BrnMall.Core.BMACache.Remove(CacheKeys.MALL_CATEGORY_BRANDLIST + productInfo.CateId);
                    BrnMall.Core.BMACache.Remove(CacheKeys.MALL_BRAND_CATEGORYLIST + productInfo.BrandId);
                }
                ...
                productInfo.MemberPrice = model.MemberPrice;
                productInfo.VipPrice = model.VipPrice;
                ...
                string msg = "商品修改成功";
                string advertPositions= Adverts.GetAdvertPositionsByPid(pid);
                if (!string.IsNullOrWhiteSpace(advertPositions))
                {
                    msg += ",该商品存在于广告位：" + advertPositions + "中，若修改了重要信息请及时修改对应广告信息！";
                }

                AddMallAdminLog("修改商品", "修改商品,商品ID为:" + pid);
                return PromptView(msg);
            }
        ...
        public ActionResult OutSaleProduct(int[] pidList)
        {
            ...
            foreach (var pid in pidList)
            {
                ...
                if (!string.IsNullOrWhiteSpace(advertPositions))
                {
                    if (WorkContext.IsHttpAjax)
                    {
                        return Content("商品id：" + pid + "存在于广告位：" + advertPositions + "中，请先撤下该广告才可下架该商品！");
                    }
                    else
                    {
                        return PromptView("商品id：" + pid + "存在于广告位：" + advertPositions + "中，请先撤下该广告才可下架该商品！");
                    }
                }
            }
            ...
            if (result)
            {
                ...
                return AjaxResult("succ", "下架成功！");
            }
            else
            {
                return AjaxResult("error", "下架失败，请联系管理员！");
            }
        }
        ...
        public ActionResult RecycleProduct(int[] pidList)
        {
            ...
            foreach (var pid in pidList)
            {
                string advertPositions = Adverts.GetAdvertPositionsByPid(pid).Trim();
                if (!string.IsNullOrWhiteSpace(advertPositions))
                {
                    return PromptView("商品id：" + pid + "存在于广告位：" + advertPositions + "中，请先撤下广告位才可删除该商品！");
                }
            }
        ...
        public ActionResult DelProduct(int[] pidList)
        {
            ...
            foreach (var pid in pidList)
            {
                ...
                if (!string.IsNullOrWhiteSpace(advertPositions))
                {
                    return PromptView("商品id：" + pid + "存在于广告位：" + advertPositions + "中，请先撤下该广告才可删除该商品！");
                }
            }
        ...
        /// <param name="vipPrice"></param>
        /// <returns></returns>
        public ActionResult UpdateProductVipPrice(int pid = -1, decimal vipPrice = 0.00M)
        {
            bool result = AdminProducts.UpdateProductVipPrice(pid, vipPrice);
            if (result)
            {
                AddMallAdminLog("修改商品本店队员价", "修改商品本店队员价,商品ID和队员价为:" + pid + "_" + vipPrice);
                return Content("1");
            }
            else
            {
