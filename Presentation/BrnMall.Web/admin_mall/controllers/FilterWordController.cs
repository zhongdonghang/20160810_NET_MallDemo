    ...
    public partial class FilterWordController : BaseMallAdminController
    {
        ...
        public ActionResult Add(FilterWordModel model)
        {
            if (!string.IsNullOrWhiteSpace(model.Match) && AdminFilterWords.GetFilterWordList().Find(x => x.Match == model.Match.Trim()) != null)
                ModelState.AddModelError("Match", "禁止词已经存在");
            ...
            if (ModelState.IsValid)
            {
                ...
                AddMallAdminLog("添加禁止词", "添加禁止词,禁止词为:" + model.Match);
                return PromptView("禁止词添加成功");
            }
            ViewData["referer"] = MallUtils.GetMallAdminRefererCookie();
            return View(model);
        }
        ...
        public ActionResult Edit(FilterWordModel model, int id = -1)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                AddMallAdminLog("修改禁止词", "修改禁止词,禁止词ID为:" + id);
                return PromptView("禁止词修改成功");
            }
        ...
        public ActionResult Del(int[] idList)
        {
            ...
            AddMallAdminLog("删除禁止词", "删除禁止词,禁止词ID为:" + CommonHelper.IntArrayToString(idList));
            return PromptView("禁止词删除成功");
        }
    }
}
