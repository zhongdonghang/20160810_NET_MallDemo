...
$(function () {
    ...
    if (productPageType != 2 && cateId > 0) {
        ...
        //队员价
        var oldvipPriceInputValue = 0;
        $(".vippriceinput").focus(function () {
            var vipPriceInputObj = $(this);
            oldvipPriceInputValue = vipPriceInputObj.val();
            vipPriceInputObj.val("");
            vipPriceInputObj.attr("class", "selectedsortinput");
        });
        $(".vippriceinput").blur(function () {
            var vipPriceInputObj = $(this);
            if (vipPriceInputObj.val() == "") {
                vipPriceInputObj.val(oldvipPriceInputValue)
            }
            else {
                var reg = /^\d+(\.\d{1,2})?$/;
                if (!reg.test(vipPriceInputObj.val())) {
                    vipPriceInputObj.val(oldvipPriceInputValue).attr("class", "selectedsortinput");
                    alert("只能输入正数且最多两位小数！")
                    return;
                }
                else {
                    if (oldvipPriceInputValue != vipPriceInputObj.val()) {
                        $.jBox.tip("正在更新...", 'loading');
                        $.get(vipPriceInputObj.attr("url") + "&vipPrice=" + vipPriceInputObj.val() + "&t=" + new Date(), function (data, textStatus) {
                            if (data != "0") {
                                $.jBox.tip('更新成功！', 'success');
                            } else {
                                vipPriceInputObj.val(oldvipPriceInputValue);
            ...
            vipPriceInputObj.attr("class", "unselectedsortinput");
        });
        ...
        $(".isbestspan").click(function () {
            ...
            var isBestUrl = "";
            if (isBest == "0") {
                alertMessage = "您确认要设置此商品为推荐吗？";
                isBestUrl = isBestSpanObj.attr("url") + "&state=1" + "&t=" + new Date();
            }
            else if (isBest == "1") {
                alertMessage = "您确认要取消此商品为推荐吗？";
