...
function consultProduct(uid, pid) {
    ...
    document.getElementById("sumitReviewBtn").disabled = true;
...
function verifyConsultProduct(uid, pid, consultTypeId, consultMessage, verifyCode) {
    ...
    if (consultTypeId < 1) {
        Showbo.Msg.alert('请选择咨询类型');
        return false;
    }
    if (consultMessage.lenth < 1) {
        Showbo.Msg.alert('请填写咨询内容');
        return false;
    }
    if (consultMessage.length > 100) {
        Showbo.Msg.alert('咨询内容内容太长');
        return false;
    }
...
function consultProductResponse(data) {
    ...
    if (result.state == "success") {
        Showbo.Msg.alert("您的咨询我们已经收到，我们会尽快给您回复");
        setTimeout(function () {
    ...
    document.getElementById("sumitReviewBtn").disabled = false;
}
