var Showbo={author:'wei',homepage:''};  
//是否为ie浏览器  
...
Showbo.Msg={  
    ...
    InitMsg:function(width){  
      ...
      html='<div class="top"><div class="mbright"><div class="title" id="dvMsgTitle"></div></div></div>'+  
        '<div class="body"><div class="mbright"><div class="ct" id="dvMsgCT"></div></div></div>'+  
        '<div class="bottom" id="dvMsgBottom"><div class="mbright"><div class="btn" id="dvMsgBtns"></div></div></div>';  
...
//      cfg:{title:'',msg:'',wait:true,icon:'默认为信息',buttons:{yes:'',no:''},fn:function(btn){回调函数,btn为点击的按钮，可以为yes，no},width:显示层的宽}  
      ...
      if(cfg.wait){  
        ...
        if(cfg.buttons.no)this.dvBtns.appendChild(this.createBtn('no',cfg.buttons.no,null));  
      }  
    ...
    onResize:function(isResize){
       ...
       Showbo.Msg.dvMsgBox.style.position = "absolute";
       var top = ($(window).height() - 200) / 2;
       var left = ($(window).width() - 300) / 2;  
       var scrollTop = $(document).scrollTop();
       var scrollLeft = $(document).scrollLeft();
       Showbo.Msg.dvMsgBox.style.top = top + scrollTop + 'px';
       Showbo.Msg.dvMsgBox.style.left = left + scrollLeft + 'px';  
    }  
