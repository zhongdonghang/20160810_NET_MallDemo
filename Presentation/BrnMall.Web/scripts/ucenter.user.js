...
function openAddShipAddressBlock() {
    ...
    window.location.hash = "#shipAddressBlock";
}
...
function addShipAddressResponse(data) {
    ...
    if (result.state == "success") {
        ...
        Showbo.Msg.alert('配送地址的数量已经达到系统所允许的最大值');
    }
...
function verifyEditUser(userName, nickName, realName, address, bio) {
    if (userName != undefined) {
        if (userName.length > 10) {
            Showbo.Msg.alert('用户名长度不能大于10');
            return false;
        }
    }
    if (nickName.length > 10) {
        Showbo.Msg.alert('昵称长度不能大于10');
        return false;
    }
    if (realName.length > 5) {
        Showbo.Msg.alert('真实姓名长度不能大于10');
        return false;
    }
    if (address.length > 75) {
        Showbo.Msg.alert('详细地址长度不能大于75');
        return false;
    }
    if (bio.length > 150) {
        Showbo.Msg.alert('简介长度不能大于150');
        return false;
    }
    return true;
}
