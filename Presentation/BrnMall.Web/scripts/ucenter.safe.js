...
function updatePayPassword() {
    var updatePayPasswordForm = document.forms["updatePayPasswordForm"];

    var v = updatePayPasswordForm.elements["v"].value;
    var password = updatePayPasswordForm.elements["paypassword"].value;
    var confirmPwd = updatePayPasswordForm.elements["payconfirmPwd"].value;
    var verifyCode = updatePayPasswordForm.elements["payverifyCode"].value;

    if (password.length == 0) {
        Showbo.Msg.alert('请输入支付密码');
        return;
    }
    if (password != confirmPwd) {
        Showbo.Msg.alert('两次输入的密码不一样');
