...
namespace BrnMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public void UpdateOnlineUserSid(int olId, string sid)
        {
            string commandText = string.Format(" UPDATE [{0}onlineusers] SET [sid]='{1}' WHERE [olid]={2} "
        ...
        /// <param name="ip">ip</param>
        /// <returns></returns>
        public IDataReader GetOnlineGuestByIP(string ip)
        {
            string commandText = string.Format("SELECT TOP 1 [olid],[uid],[sid],[nickname],[ip],[regionid],[updatetime] FROM [{0}onlineusers] WHERE [ip]='{1}' AND [uid]=-1"
                ,RDBSHelper.RDBSTablePre
                ,ip);
        ...
        public int CreateUser(UserInfo userInfo)
        {
            DbParameter[] parms = {
									   ...
									   GenerateInParam("@useramount",SqlDbType.Decimal,8,userInfo.UserAmount),
									   GenerateInParam("@frozenamount",SqlDbType.Decimal,8,userInfo.FrozenAmount),
        ...
        public void UpdatePartUser(PartUserInfo partUserInfo)
        {
            DbParameter[] parms = {
									   ...
									   GenerateInParam("@useramount",SqlDbType.Decimal,8,partUserInfo.UserAmount),
									   GenerateInParam("@frozenamount",SqlDbType.Decimal,8,partUserInfo.FrozenAmount),
        ...
        public DataTable AdminGetUserList(int pageSize, int pageNumber, string condition, string sort)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} [{1}users].[uid],[{1}users].[username],[{1}users].[email],[{1}users].[mobile],[{1}users].[userrid],[{1}users].[storeid],[{1}users].[mallagid],[{1}users].[nickname],[{1}users].[useramount],[{1}users].[frozenamount],[{1}users].[rankcredits],[{1}userdetails].[lastvisittime],[{1}userdetails].[lastvisitip],[{1}userdetails].[registertime],[{1}userdetails].[gender],[{1}userdetails].[realname],[{1}userranks].[title] AS [utitle],[{1}malladmingroups].[title] AS [atitle] FROM [{1}users] LEFT JOIN [{1}userdetails] ON [{1}userdetails].[uid] = [{1}users].[uid]  LEFT JOIN [{1}userranks] ON [{1}userranks].[userrid]=[{1}users].[userrid]  LEFT JOIN [{1}malladmingroups] ON [{1}malladmingroups].[mallagid]=[{1}users].[userrid] ORDER BY {2}",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                sort);

                else
                    commandText = string.Format("SELECT TOP {0} [{1}users].[uid],[{1}users].[username],[{1}users].[email],[{1}users].[mobile],[{1}users].[userrid],[{1}users].[storeid],[{1}users].[mallagid],[{1}users].[nickname],[{1}users].[useramount],[{1}users].[frozenamount],[{1}users].[rankcredits],[{1}userdetails].[lastvisittime],[{1}userdetails].[lastvisitip],[{1}userdetails].[registertime],[{1}userdetails].[gender],[{1}userdetails].[realname],[{1}userranks].[title] AS [utitle],[{1}malladmingroups].[title] AS [atitle] FROM [{1}users] LEFT JOIN [{1}userdetails] ON [{1}userdetails].[uid] = [{1}users].[uid]  LEFT JOIN [{1}userranks] ON [{1}userranks].[userrid]=[{1}users].[userrid]  LEFT JOIN [{1}malladmingroups] ON [{1}malladmingroups].[mallagid]=[{1}users].[userrid] WHERE {2} ORDER BY {3}",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition,
                                                sort);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} [{1}users].[uid],[{1}users].[username],[{1}users].[email],[{1}users].[mobile],[{1}users].[userrid],[{1}users].[storeid],[{1}users].[mallagid],[{1}users].[nickname],[{1}users].[useramount],[{1}users].[frozenamount],[{1}users].[rankcredits],[{1}userdetails].[lastvisittime],[{1}userdetails].[lastvisitip],[{1}userdetails].[registertime],[{1}userdetails].[gender],[{1}userdetails].[realname],[{1}userranks].[title] AS [utitle],[{1}malladmingroups].[title] AS [atitle] FROM [{1}users],[{1}userdetails],[{1}userranks],[{1}malladmingroups]  WHERE [{1}userdetails].[uid] = [{1}users].[uid] AND  [{1}userranks].[userrid]=[{1}users].[userrid] AND  [{1}malladmingroups].[mallagid]=[{1}users].[mallagid] AND [{1}users].[uid] < (SELECT min([uid])  FROM (SELECT TOP {2} [uid] FROM [{1}users] ORDER BY {3}) AS temp ) ORDER BY {3}",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                sort);
                else
                    commandText = string.Format("SELECT TOP {0} [{1}users].[uid],[{1}users].[username],[{1}users].[email],[{1}users].[mobile],[{1}users].[userrid],[{1}users].[storeid],[{1}users].[mallagid],[{1}users].[nickname],[{1}users].[useramount],[{1}users].[frozenamount],[{1}users].[rankcredits],[{1}userdetails].[lastvisittime],[{1}userdetails].[lastvisitip],[{1}userdetails].[registertime],[{1}userdetails].[gender],[{1}userdetails].[realname],[{1}userranks].[title] AS [utitle],[{1}malladmingroups].[title] AS [atitle] FROM [{1}users],[{1}userdetails],[{1}userranks],[{1}malladmingroups]  WHERE [{1}userdetails].[uid] = [{1}users].[uid] AND  [{1}userranks].[userrid]=[{1}users].[userrid] AND  [{1}malladmingroups].[mallagid]=[{1}users].[mallagid] AND [{1}users].[uid] < (SELECT min([uid])  FROM (SELECT TOP {2} [uid] FROM [{1}users] WHERE {4} ORDER BY {3}) AS temp ) AND {4} ORDER BY {3}",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                sort,
        ...
        public void CreateUserIntroduceId(int uid, int IntroduceId)
        {
            DbParameter[] parms = {
									   GenerateInParam("@uid",SqlDbType.Int,4, uid),
									   GenerateInParam("@introduceId",SqlDbType.Int,4, IntroduceId)
								   };
            string commandText = string.Format("INSERT INTO [{0}userrelations]([uid],[introduceid]) VALUES(@uid,@introduceId)",
                                                RDBSHelper.RDBSTablePre);
        ...
        public void UpdatePayPasswordByUid(int uid, string password)
        {
            DbParameter[] parms = {
									   GenerateInParam("@uid",SqlDbType.Int,4, uid),
									   GenerateInParam("@password",SqlDbType.Char,32, password)
								   };
            string commandText = string.Format("UPDATE [{0}userpaypassword] SET [password]=@password WHERE [uid]=@uid", RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public void CreatePayPassword(int uid, string password, string salt)
        {
            DbParameter[] parms = {
									   GenerateInParam("@uid",SqlDbType.Int,4, uid),
									   GenerateInParam("@salt",SqlDbType.Char,6, salt),
									   GenerateInParam("@password",SqlDbType.Char,32, password)
								   };
            string commandText = string.Format("INSERT INTO [{0}userpaypassword] ([uid],[salt],[password]) VALUES(@uid,@salt,@password)", RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText,parms);
        }
        ...
        /// <param name="password">密码</param>
        /// <param name="salt">盐值</param>
        public IDataReader GetUserPayPasswordByUid(int uid)
        {
            string commandText = string.Format("SELECT [uid],[salt],[password] FROM [{0}userpaypassword] WHERE [uid]={1}", RDBSHelper.RDBSTablePre, uid);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText);
        }
        ...
        /// </summary>
        /// <param name="uid">用户id.</param>
        /// <param name="wxnames">微信号们</param>
        public void UpdateUserWxUnionIdsByUid(int uid, string wxnames)
        {
            DbParameter[] parms = {
									   GenerateInParam("@uid",SqlDbType.Int,4, uid),
									   GenerateInParam("@wxunionids",SqlDbType.NChar,250, wxnames)
            };
            string commandText = string.Format("UPDATE [{0}users] SET [wxunionids]=@wxunionids WHERE [uid]=@uid", RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        /// <param name="uid">用户id.</param>
        /// <param name="avatar">头像</param>
        public void UpdateUserAvatarByUid(int uid, string avatar)
        {
            DbParameter[] parms = {
									   GenerateInParam("@uid",SqlDbType.Int,4, uid),
									   GenerateInParam("@avatar",SqlDbType.Char,40, avatar)
        ...
        public  IDataReader GetPartUserInfoByWeixinUnid(string wxunionid)
        {
            string commandText = string.Format("SELECT [uid],[username],[email],[mobile],[password],[userrid],[storeid],[mallagid],[nickname],[avatar],[useramount],[frozenamount],[rankcredits],[verifyemail],[verifymobile],[liftbantime],[salt],[wxunionids] FROM [{0}users] WHERE [wxunionids] like '%{1}%';", RDBSHelper.RDBSTablePre,wxunionid);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText);
        }
        ...
        /// <param name="uid"></param>
        /// <returns></returns>
        public int GetMyIntroducer(int uid)
        {
            DbParameter[] parms = {
									GenerateInParam("@uid",SqlDbType.Int,4, uid)
								   };
            string commandText = string.Format("SELECT [introduceid] FROM [{0}userrelations] WHERE [uid]=@uid",
                                                RDBSHelper.RDBSTablePre);
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.Text, commandText, parms));
        }
        ...
        /// <param name="uid"></param>
        /// <returns></returns>
        public DataTable GetMyIntroducerList(int uid)
        {
            string commandText = string.Format("select [u].[nickname],[u].[username],[d].[registertime] from [{0}users] u left join [{0}userrelations] r on [u].[uid]=[r].[uid] left join [{0}userdetails] d on [u].[uid]=[d].[uid] where [r].[introduceid]={1}",
                                                RDBSHelper.RDBSTablePre, uid);
        ...
        {
            string commandText = string.Format("SELECT {1} FROM [{0}userranks] ORDER BY [system] DESC,[creditslower] ASC",
                                                RDBSHelper.RDBSTablePre,
        ...
        public void CreateUserRank(UserRankInfo userRankInfo)
        {
            DbParameter[] parms = {
                                        ...
                                        GenerateInParam("@discount", SqlDbType.Int,4,userRankInfo.Discount)
                                    };
            string commandText = string.Format("INSERT INTO [{0}userranks]([system],[title],[avatar],[creditslower],[creditsupper],[limitdays],[discount]) VALUES(@system,@title,@avatar,@creditslower,@creditsupper,@limitdays,@discount)",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public void UpdateUserRank(UserRankInfo userRankInfo)
        {
            ...
            string commandText = string.Format("UPDATE [{0}userranks] SET [system]=@system,[title]=@title,[avatar]=@avatar,[creditslower]=@creditslower,[creditsupper]=@creditsupper,[limitdays]=@limitdays,[discount]=@discount WHERE [userrid]=@userrid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
