...
namespace BrnMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public DataTable SumStoreReview(int storeId, DateTime? startTime, DateTime? endTime)
        {
            ...
            commandText.AppendFormat("SELECT [descriptionstar] AS [star],SUM([descriptionstar]) AS [count] FROM [{0}storereviews] WHERE [storeid]={1}",RDBSHelper.RDBSTablePre,storeId);
            if(startTime!=null)
            {
             commandText.AppendFormat(" AND [reviewtime]>={0}",startTime);
            }
            if(endTime!=null)
            {
             commandText.AppendFormat(" AND [reviewtime]<={0}",startTime);
            }
            commandText.AppendFormat(" GROUP BY [descriptionstar] UNION ALL SELECT [star]=0,[count]=0 UNION ALL SELECT [servicestar] AS [star],SUM([servicestar]) AS [count] FROM [{0}storereviews] WHERE [storeid]={1}",RDBSHelper.RDBSTablePre,storeId);
           if(startTime!=null)
            {
             commandText.AppendFormat(" AND [reviewtime]>={0}",startTime);
            }
            if(endTime!=null)
            {
             ...
             commandText.AppendFormat(" GROUP BY [servicestar] UNION ALL SELECT [star]=0,[count]=0 UNION ALL SELECT [shipstar] AS [star],SUM([shipstar]) AS [count] FROM [{0}storereviews] WHERE [storeid]={1}",RDBSHelper.RDBSTablePre,storeId);
             if(startTime!=null)
            {
             commandText.AppendFormat(" AND [reviewtime]>={0}",startTime);
            }
            if(endTime!=null)
            {
             ...
             commandText.AppendFormat(" GROUP BY [shipstar]");
        ...
        public int CreateStoreShipTemplate(StoreShipTemplateInfo storeShipTemplateInfo)
        {
            DbParameter[] parms = {
                                        ...
                                        GenerateInParam("@freeprice", SqlDbType.Decimal,8, storeShipTemplateInfo.FreePrice),
            ...
            string commandText = string.Format("INSERT INTO [{0}storeshiptemplates]([storeid],[free],[type],[title],[freeprice]) VALUES(@storeid,@free,@type,@title,@freeprice);SELECT SCOPE_IDENTITY();",
                                                RDBSHelper.RDBSTablePre);
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.Text, commandText, parms));
        }
        ...
        public void UpdateStoreShipTemplate(StoreShipTemplateInfo storeShipTemplateInfo)
        {
            ...
            string commandText = string.Format("UPDATE [{0}storeshiptemplates] SET [storeid]=@storeid,[free]=@free,[type]=@type,[title]=@title,[freeprice]=@freeprice WHERE [storestid]=@storestid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
