...
namespace BrnMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public DataTable AdminGetAdvertList(int pageSize, int pageNumber, int adPosId)
        {
            ...
            {
                if (adPosId < 1)
                    commandText = string.Format("SELECT [temp1].[adid],[temp1].[clickcount],[temp1].[adposid],[temp1].[type],[temp1].[title] AS [atitle],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[displayorder],[temp2].[title] AS [aptitle] FROM (SELECT TOP {0} [adid],[clickcount],[adposid],[type],[title],[starttime],[endtime],[state],[displayorder] FROM [{1}adverts] ORDER BY [adposid] DESC,[displayorder] ASC,[adid] DESC) AS [temp1] LEFT JOIN [{1}advertpositions] AS [temp2] ON [temp1].[adposid]=[temp2].[adposid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT [temp1].[adid],[temp1].[clickcount],[temp1].[adposid],[temp1].[type],[temp1].[title] AS [atitle],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[displayorder],[temp2].[title] AS [aptitle] FROM (SELECT TOP {0} [adid],[clickcount],[adposid],[type],[title],[starttime],[endtime],[state],[displayorder] FROM [{1}adverts] WHERE [adposid]={2} ORDER BY [adposid] DESC,[displayorder] ASC,[adid] DESC) AS [temp1] LEFT JOIN [{1}advertpositions] AS [temp2] ON [temp1].[adposid]=[temp2].[adposid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                adPosId);
            }
            ...
            {
                if (adPosId < 1)
                    commandText = string.Format("SELECT [temp1].[adid],[temp1].[clickcount],[temp1].[adposid],[temp1].[type],[temp1].[title] AS [atitle],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[displayorder],[temp2].[title] AS [aptitle] FROM (SELECT TOP {0} [adid],[clickcount],[adposid],[type],[title],[starttime],[endtime],[state],[displayorder] FROM [{1}adverts] WHERE [adid] NOT IN (SELECT TOP {2} [adid] FROM [{1}adverts] ORDER BY [adposid] DESC,[displayorder] ASC,[adid] DESC) ORDER BY [adposid] DESC,[displayorder] DESC,[adid] DESC) AS [temp1] LEFT JOIN [{1}advertpositions] AS [temp2] ON [temp1].[adposid]=[temp2].[adposid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize);
                else
                    commandText = string.Format("SELECT [temp1].[adid],[temp1].[clickcount],[temp1].[adposid],[temp1].[type],[temp1].[title] AS [atitle],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[displayorder],[temp2].[title] AS [aptitle] FROM (SELECT TOP {0} [adid],[clickcount],[adposid],[type],[title],[starttime],[endtime],[state],[displayorder] FROM [{1}adverts] WHERE [adposid]={3} AND [adid] NOT IN (SELECT TOP {2} [adid] FROM [{1}adverts] WHERE [adposid]={3} ORDER BY [adposid] DESC,[displayorder] ASC,[adid] DESC) ORDER BY [adposid] DESC,[displayorder] DESC,[adid] DESC) AS [temp1] LEFT JOIN [{1}advertpositions] AS [temp2] ON [temp1].[adposid]=[temp2].[adposid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                adPosId);
        ...
        public IDataReader GetAdvertList(int adPosId, DateTime nowTime)
        {
            ...
            string commandText = string.Format("SELECT {0} FROM [{1}adverts] WHERE [adposid]=@adposid AND [state]=1 AND [starttime]<=@nowtime AND [endtime]>@nowtime ORDER BY [displayorder] ASC,[adid] DESC",
                                                RDBSFields.ADVERTS,
                                                RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, param);
        }
        ...
        /// <returns></returns>
        public IDataReader GetAdvertPositionsByPid(int pid)
        {
            string pidstr = pid.ToString();
            DbParameter[] param = {
                                     GenerateInParam("@extfield5",SqlDbType.NChar, 50, pidstr)
                                   };
            string commandText = string.Format("SELECT [title] FROM [{0}advertpositions] WHERE [adposid] IN(SELECT [adposid] FROM [{0}adverts] WHERE [extfield5]=@extfield5 AND [state]=1)",
                                                RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, param);
        }
        ...
        public void CreateShipCompany(ShipCompanyInfo shipCompanyInfo)
        {
            DbParameter[] param = {
                                    ...
                                    GenerateInParam("@code",SqlDbType.NChar, 30, shipCompanyInfo.Code)
                                   };
            string commandText = String.Format("INSERT INTO [{0}shipcompanies] ([name],[displayorder],[code]) VALUES(@name,@displayorder,@code)",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, param);
        }
        ...
        public void UpdateShipCompany(ShipCompanyInfo shipCompanyInfo)
        {
            ...
            string commandText = String.Format("UPDATE [{0}shipcompanies] SET [name]=@name,[displayorder]=@displayorder,[code]=@code WHERE [shipcoid]=@shipcoid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, param);
        }
        ...
        public DataTable AdminGetCreditLogList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[logid],[temp1].[uid],[temp1].[useramount],[temp1].[frozenamount],[temp1].[rankcredits],[temp1].[action],[temp1].[actioncode],[temp1].[actiontime],[temp1].[actiondes],[temp1].[operator],[temp2].[username] AS [rusername],[temp3].[username] AS [ousername] FROM (SELECT TOP {0} {2} FROM [{1}creditlogs] ORDER BY [logid] DESC) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] LEFT JOIN [{1}users] AS [temp3] ON [temp1].[operator]=[temp3].[uid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                RDBSFields.CREDITLOGS);

                else
                    commandText = string.Format("SELECT [temp1].[logid],[temp1].[uid],[temp1].[useramount],[temp1].[frozenamount],[temp1].[rankcredits],[temp1].[action],[temp1].[actioncode],[temp1].[actiontime],[temp1].[actiondes],[temp1].[operator],[temp2].[username] AS [rusername],[temp3].[username] AS [ousername] FROM (SELECT TOP {0} {2} FROM [{1}creditlogs] WHERE [3] ORDER BY [logid] DESC) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] LEFT JOIN [{1}users] AS [temp3] ON [temp1].[operator]=[temp3].[uid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                RDBSFields.CREDITLOGS,
                                                condition);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[logid],[temp1].[uid],[temp1].[useramount],[temp1].[frozenamount],[temp1].[rankcredits],[temp1].[action],[temp1].[actioncode],[temp1].[actiontime],[temp1].[actiondes],[temp1].[operator],[temp2].[username] AS [rusername],[temp3].[username] AS [ousername] FROM (SELECT TOP {0} {2} FROM [{1}creditlogs] WHERE [logid] < (SELECT MIN([logid]) FROM (SELECT TOP {3} [logid] FROM [{1}creditlogs] ORDER BY [logid] DESC) AS [t]) ORDER BY [logid] DESC) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] LEFT JOIN [{1}users] AS [temp3] ON [temp1].[operator]=[temp3].[uid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                RDBSFields.CREDITLOGS,
                                                (pageNumber - 1) * pageSize);
                else
                    commandText = string.Format("SELECT [temp1].[logid],[temp1].[uid],[temp1].[useramount],[temp1].[frozenamount],[temp1].[rankcredits],[temp1].[action],[temp1].[actioncode],[temp1].[actiontime],[temp1].[actiondes],[temp1].[operator],[temp2].[username] AS [rusername],[temp3].[username] AS [ousername] FROM (SELECT TOP {0} {2} FROM [{1}creditlogs] WHERE [logid] < (SELECT MIN([logid]) FROM (SELECT TOP {3} [logid] FROM [{1}creditlogs] WHERE {4} ORDER BY [logid] DESC) AS [t]) AND {4} ORDER BY [logid] DESC) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] LEFT JOIN [{1}users] AS [temp3] ON [temp1].[operator]=[temp3].[uid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                RDBSFields.CREDITLOGS,
                                                (pageNumber - 1) * pageSize,
        ...
        public void SendCredits(int userRid, CreditLogInfo creditLogInfo)
        {
            DbParameter[] parms = {
									   ...
									   GenerateInParam("@useramount",SqlDbType.Decimal,8,creditLogInfo.UserAmount),
									   GenerateInParam("@frozenamount",SqlDbType.Decimal,8,creditLogInfo.FrozenAmount),
        ...
        /// <param name="pageNumber">当前页数</param>
        /// <returns></returns>
        public IDataReader GetUserAmountLogList(int uid, int pageSize, int pageNumber)
        {
            string actions = ((int)CreditAction.SendFrozenAmount).ToString() + "," + ((int)CreditAction.ThawAmount).ToString() + "," + ((int)CreditAction.RerurnOrderAmount).ToString() + "," + ((int)CreditAction.CodOrderAmount).ToString() + "," + ((int)CreditAction.WithdrawCash).ToString() + "," + ((int)CreditAction.UserLoadCash).ToString() + "," + ((int)CreditAction.UserOrderCreditPay).ToString();
            
            DbParameter[] parms = {
									   GenerateInParam("@uid",SqlDbType.Int,4,uid),
									   GenerateInParam("@pagesize",SqlDbType.Int,4,pageSize),
									   GenerateInParam("@pagenumber",SqlDbType.Int,4,pageNumber),
                                       GenerateInParam("@actions",SqlDbType.NChar,50,actions.Trim())
								   };
            return RDBSHelper.ExecuteReader(CommandType.StoredProcedure,
        ...
        /// <param name="uid">用户id</param>
        /// <returns></returns>
        public int GetUserAmountLogCount(int uid)
        {
            string actions = ((int)CreditAction.SendFrozenAmount).ToString() + "," + ((int)CreditAction.ThawAmount).ToString() + "," + ((int)CreditAction.RerurnOrderAmount).ToString() + "," + ((int)CreditAction.CodOrderAmount).ToString() + "," + ((int)CreditAction.WithdrawCash).ToString() + "," + ((int)CreditAction.UserLoadCash).ToString() + "," + ((int)CreditAction.UserOrderCreditPay).ToString();
        ...
        /// <returns></returns>
        public IDataReader GetWithdrawalLogById(int id)
        {
             StringBuilder commandText = new StringBuilder();
             commandText.AppendFormat("SELECT {1} FROM [{0}withdrawalLogs]  WHERE [recordid]={2}", RDBSHelper.RDBSTablePre, RDBSFields.WITHDRAWALLOGS, id);
        ...
        /// <returns></returns>
        public int GetWithdrawalLogCount(int uid, int state, int paytype)
        {
            StringBuilder commandText = new StringBuilder();
            commandText.AppendFormat("SELECT COUNT(recordid) FROM [{0}withdrawalLogs] WHERE [recordid]>0", RDBSHelper.RDBSTablePre);
            if (uid > 0)
            {
                commandText.AppendFormat(" AND [uid]={0}",uid);
            }
            if (state > 0)
            {
                commandText.AppendFormat(" AND [state]={0}", state);
            }
            if (paytype > 0)
            {
                commandText.AppendFormat(" AND [paytype]={0}", paytype);
            }
        ...
        /// <returns></returns>
        public IDataReader GetWithdrawalLogList(int uid, int state, int paytype, int pagenumber, int pagesize)
        {
            StringBuilder commandText = new StringBuilder();

            if (pagenumber == 1)
            {
                commandText.AppendFormat("SELECT TOP {1} {2} FROM [{0}withdrawalLogs] WHERE [recordid]>0", RDBSHelper.RDBSTablePre, pagesize, RDBSFields.WITHDRAWALLOGS);
                if (uid > 0)
                {
                    commandText.AppendFormat(" AND [uid]={0}", uid);
                }
                ...
                commandText.AppendFormat(" ORDER BY [recordid] DESC");
            }
            else
            {
                commandText.AppendFormat("SELECT TOP {1} {2} FROM [{0}withdrawalLogs] WHERE [recordid] NOT IN (", RDBSHelper.RDBSTablePre, pagesize, RDBSFields.WITHDRAWALLOGS);

                commandText.AppendFormat("SELECT TOP {1} [recordid] FROM [{0}withdrawalLogs] WHERE [recordid]>0", RDBSHelper.RDBSTablePre, (pagenumber - 1) * pagesize);
        ...
        public void CreateWithdrawalLog(WithdrawalLogInfo info)
        {
            DbParameter[] parms = {
									   GenerateInParam("@uid",SqlDbType.Int,16,info.Uid),
									   GenerateInParam("@state",SqlDbType.SmallInt,4,info.State),
									   GenerateInParam("@paytype",SqlDbType.SmallInt,4,info.PayType),
									   GenerateInParam("@payaccount",SqlDbType.NChar,50,info.PayAccount),
									   GenerateInParam("@applyamount",SqlDbType.Decimal,18,info.ApplyAmount),
									   GenerateInParam("@applyremark",SqlDbType.NChar,100,info.ApplyRemark),
									   GenerateInParam("@phone",SqlDbType.NChar,20,info.Phone),
									   GenerateInParam("@applytime",SqlDbType.DateTime,8,info.ApplyTime),
									   GenerateInParam("@operatoruid",SqlDbType.Int,16,info.OperatorUid),
									   GenerateInParam("@operattime",SqlDbType.DateTime,8,info.OperatTime),
									   GenerateInParam("@reason",SqlDbType.NChar,100,info.Reason)
                                   };

            string commandText = string.Format("INSERT INTO [{0}withdrawalLogs]([uid],[state],[paytype],[payaccount],[applyamount],[applyremark],[phone],[applytime],[operatoruid],[operattime],[reason]) VALUES(@uid,@state,@paytype,@payaccount,@applyamount,@applyremark,@phone,@applytime,@operatoruid,@operattime,@reason)",
                                                   RDBSHelper.RDBSTablePre);
        ...
        /// </summary>
        /// <param name="info"></param>
        public void UpdateWithdrawalLog(WithdrawalLogInfo info)
        {
            DbParameter[] parms = {
									   GenerateInParam("@recordid",SqlDbType.Int,16,info.RecordId),
									   GenerateInParam("@uid",SqlDbType.Int,16,info.Uid),
									   GenerateInParam("@state",SqlDbType.SmallInt,4,info.State),
									   GenerateInParam("@paytype",SqlDbType.SmallInt,4,info.PayType),
									   GenerateInParam("@payaccount",SqlDbType.NChar,50,info.PayAccount),
            ...
            string commandText = string.Format("UPDATE [{0}withdrawalLogs] SET [uid]=@uid,[state]=@state,[paytype]=@paytype,[payaccount]=@payaccount,[applyamount]=@applyamount,[applyremark]=@applyremark,[phone]=@phone,[applytime]=@applytime,[operatoruid]=@operatoruid,[operattime]=@operattime,[reason]=@reason WHERE [recordid]=@recordid",
                                                   RDBSHelper.RDBSTablePre);
        ...
        /// <param name="type">类型</param>
        /// <returns></returns>
        public IDataReader GetOrderAmountLog(int oid, int type)
        {
            string commandText = string.Format("select {1} from [{0}creditlogs] where [actioncode]={2} and action={3}",
                                               RDBSHelper.RDBSTablePre,
                                               RDBSFields.CREDITLOGS,
                                               oid,
                                               type);
        ...
        #region 短信日志
        public void CreateSMSLog(SMSLogInfo logInfo)
        { 
            DbParameter[] param = {
                                     GenerateInParam("@phone",SqlDbType.NVarChar, 20, logInfo.Phone),
                                     GenerateInParam("@type",SqlDbType.SmallInt, 2, logInfo.Type),
                                     GenerateInParam("@sendTime",SqlDbType.DateTime, 8, logInfo.SendTime),
                                     GenerateInParam("@sendSuccess",SqlDbType.Bit, 1, logInfo.IsSendSuccess),
                                     GenerateInParam("@codeUsed",SqlDbType.Bit, 1, logInfo.CodeUsed),
                                     GenerateInParam("@smsContent",SqlDbType.NVarChar, 500, logInfo.SMSContent),
                                     GenerateInParam("@ip",SqlDbType.NVarChar, 50, logInfo.IP),
                                     GenerateInParam("@code",SqlDbType.NVarChar, 20, logInfo.Code)
                                   };
            string commandText = String.Format("INSERT INTO [{0}smslog]([phone],[type],[sendTime],[sendSuccess],[codeUsed],[smsContent],[ip],[code]) VALUES(@phone,@type,@sendTime,@sendSuccess,@codeUsed,@smsContent,@ip,@code)",RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, param);
        }
        #endregion
    }
