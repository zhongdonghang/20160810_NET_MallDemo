...
namespace BrnMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        {
            //找出该分类下的所有子分类id（包括自己）
            StringBuilder cateIds = GetAllCateIdsByParentCateId(cateId);
          
            string ids = cateIds.ToString().Trim();

            DbParameter[] parms = {
                                    GenerateInParam("@cateids", SqlDbType.NChar,500,ids)
                                   };
        ...
        public string AdminGetProductListSort(string sortColumn, string sortDirection)
        {
            if (string.IsNullOrWhiteSpace(sortColumn))
            {
                sortColumn = "[displayorder],[addtime]";
            }
            if (string.IsNullOrWhiteSpace(sortDirection))
            {
                sortDirection = "DESC";
        ...
        public DataTable AdminGetProductList(int pageSize, int pageNumber, string condition, string sort)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pid],[temp1].[storeid],[temp1].[name] AS [pname],[temp1].[shopprice],[temp1].[marketprice],[temp1].[vipprice],[temp1].[state],[temp1].[isbest],[temp1].[ishot],[temp1].[isnew],[temp1].[displayorder],[temp2].[number],[temp2].[limit],[temp3].[name] AS [sname] FROM (SELECT TOP {0} [pid],[storeid],[name],[shopprice],[marketprice],[vipprice],[state],[isbest],[ishot],[isnew],[displayorder] FROM [{1}products] ORDER BY {2}) AS [temp1] LEFT JOIN [{1}productstocks] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                sort);
        ...
        public int CreateProduct(ProductInfo productInfo)
        {
            DbParameter[] parms = {
									 ...
									 GenerateInParam("@memberprice",SqlDbType.Decimal,4,productInfo.MemberPrice),
									 GenerateInParam("@vipprice",SqlDbType.Decimal,4,productInfo.VipPrice),
            ...
            string commandText = string.Format(@"INSERT INTO [{0}products]([psn],[cateid],[brandid],[storeid],[storecid],[storestid],[skugid],[name],[shopprice],[marketprice],[costprice],[memberprice],[vipprice],[state],[isbest],[ishot],[isnew],
                                                [weight],[displayorder],[showimg],[salecount],[visitcount],[reviewcount],[star1],[star2],[star3],[star4],[star5],[addtime],[description])  
                                                VALUES(@psn,@cateid,@brandid,@storeid,@storecid,@storestid,@skugid,@name,@shopprice,@marketprice,@costprice,@memberprice,@vipprice,@state,@isbest,@ishot,@isnew,
                                                @weight,@displayorder,@showimg,@salecount,@visitcount,@reviewcount,@star1,@star2,@star3,@star4,@star5,@addtime,@description);
        ...
        public void UpdateProduct(ProductInfo productInfo)
        {
            ...
            string commandText = string.Format(@"UPDATE [{0}products] SET [psn]=@psn,[cateid]=@cateid,[brandid]=@brandid,[storeid]=@storeid,[storecid]=@storecid,[storestid]=@storestid,[skugid]=@skugid,[name]=@name,[shopprice]=@shopprice,[marketprice]=@marketprice,[costprice]=@costprice,[memberprice]=@memberprice,[vipprice]=@vipprice,
        ...
        /// </summary>
        /// <param name="pid">商品id</param>
        /// <param name="vipPrice">队员价</param>
        public bool UpdateProductVipPrice(int pid, decimal vipPrice)
        {
            DbParameter[] parms =  {
                                     GenerateInParam("@vipprice", SqlDbType.Decimal, 4, vipPrice),
                                     GenerateInParam("@pid", SqlDbType.Int, 4, pid)
                                   };
            string commandText = string.Format("UPDATE [{0}products] SET [vipprice]=@vipprice WHERE [pid]=@pid",
                                                RDBSHelper.RDBSTablePre);
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms), -1) > 0;
        }
        ...
        public IDataReader GetCategoryProductList(int pageSize, int pageNumber, int cateId, int brandId, int filterPrice, string[] catePriceRangeList, List<int> attrValueIdList, int onlyStock, int sortColumn, int sortDirection)
        {
            ...
            if (pageNumber == 1)
            {
                ...
                commandText.AppendFormat(" WHERE [p].[cateid] in ({0})", cateIds);
            ...
            {
                commandText.Append("SELECT [pid],[psn],[cateid],[brandid],[storeid],[storecid],[storestid],[skugid],[name],[shopprice],[marketprice],[costprice],[memberprice],[vipprice],[state],[isbest],[ishot],[isnew],[displayorder],[weight],[showimg],[salecount],[visitcount],[reviewcount],[star1],[star2],[star3],[star4],[star5],[addtime],[storename] FROM");
        ...
        public IDataReader GetStoreClassProductList(int pageSize, int pageNumber, int storeCid, int startPrice, int endPrice, int sortColumn, int sortDirection)
        {
            ...
            {
                commandText.Append("SELECT [pid],[psn],[cateid],[brandid],[storeid],[storecid],[storestid],[skugid],[name],[shopprice],[marketprice],[costprice],[memberprice],[vipprice],[state],[isbest],[ishot],[isnew],[displayorder],[weight],[showimg],[salecount],[visitcount],[reviewcount],[star1],[star2],[star3],[star4],[star5],[addtime] FROM");
                commandText.Append(" (SELECT ROW_NUMBER() OVER (ORDER BY ");
                switch (sortColumn)
                {
                    case 0:
                ...
                commandText.AppendFormat(") AS [rowid],[pid],[psn],[cateid],[brandid],[storeid],[storecid],[storestid],[skugid],[name],[shopprice],[marketprice],[costprice],[memberprice],[vipprice],[state],[isbest],[ishot],[isnew],[displayorder],[weight],[showimg],[salecount],[visitcount],[reviewcount],[star1],[star2],[star3],[star4],[star5],[addtime] FROM [{0}products]", RDBSHelper.RDBSTablePre);
        ...
        public DataTable AdminGetProductReviewList(int pageSize, int pageNumber, string condition, string sort)
        {
            ...
            else
            {
                if (noCondition)
                    ...
                    commandText = string.Format("SELECT [temp1].[reviewid],[temp1].[pid],[temp1].[uid],[temp1].[oprid],[temp1].[oid],[temp1].[parentid],[temp1].[state],[temp1].[storeid],[temp1].[star],[temp1].[quality],[temp1].[message],[temp1].[reviewtime],[temp1].[pname],[temp1].[pshowimg],[temp1].[buytime],[temp1].[ip],[temp2].[nickname],[temp3].[name] AS [sname] FROM (SELECT {0} FROM(SELECT TOP {3} ROW_NUMBER() OVER (ORDER BY {2}) AS [rowid],{0} FROM [{1}productreviews] WHERE {5}) AS [temp] WHERE [temp].[rowid] BETWEEN {4} AND {3}) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                RDBSFields.PRODUCT_REVIEWS,
                                                RDBSHelper.RDBSTablePre,
                                                sort,
                                                pageNumber * pageSize,
        ...
        public DataTable GetProductReviewList(DateTime startTime, DateTime endTime)
        {
            ...
            string commandText = string.Format("SELECT [pid],[star] FROM [{0}productreviews] WHERE [reviewtime]>=@starttime AND [reviewtime]<@endtime And [parentid]=0",
                                                RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText, parms).Tables[0];
        }
        ...
        /// 获得商品评价回复
        /// </summary>
        /// <param name="prwid">父评价id</param>
        /// <returns></returns>
        public  IDataReader GetProductReviewReply(int prwid)
        {
            string commandText = string.Format("SELECT {1} FROM [{0}productreviews] WHERE [parentid]={2}",
                                                RDBSHelper.RDBSTablePre
                                                ,RDBSFields.PRODUCT_REVIEWS
                                                ,prwid);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText);
        ...
        ///商品评价回复
        /// </summary>
        /// <param name="productReviewInfo"></param>
        public void ProductReviewReply(ProductReviewInfo productReviewInfo)
        {
            if (productReviewInfo.ReviewId == 0)
            {
                DbParameter[] parms = {
                                    GenerateInParam("@pid", SqlDbType.Int, 4, productReviewInfo.Pid),
                                    GenerateInParam("@uid", SqlDbType.Int, 4, productReviewInfo.Uid),
                ...
                string commandText = string.Format("INSERT INTO [{0}productreviews] ([pid],[uid],[oprid],[oid],[parentid],[state],[storeid],[star],[quality],[message],[reviewtime],[paycredits],[pname],[pshowimg],[buytime],[ip]) VALUES(@pid,@uid,@oprid,@oid,@parentid,@state,@storeid,@star,@quality,@message,@reviewtime,@paycredits,@pname,@pshowimg,@buytime,@ip)", RDBSHelper.RDBSTablePre);
                RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
            }
            else
            {
                DbParameter[] parms = {
                                    GenerateInParam("@reviewid", SqlDbType.Int, 4, productReviewInfo.ReviewId),
                                    GenerateInParam("@uid", SqlDbType.Int, 4, productReviewInfo.Uid),
                                    GenerateInParam("@message", SqlDbType.NVarChar, 200, productReviewInfo.Message),
                                    GenerateInParam("@reviewtime", SqlDbType.DateTime, 8, productReviewInfo.ReviewTime),
                                    GenerateInParam("@ip", SqlDbType.VarChar, 15, productReviewInfo.IP)
                                    };
                string commandText = string.Format("UPDATE [{0}productreviews] SET  [uid]=@uid,[message]=@message,[reviewtime]=@reviewtime,[ip]=@ip WHERE [reviewid]=@reviewid", RDBSHelper.RDBSTablePre);
        ...
        public DataTable GetSearchWordStatList(int pageSize, int pageNumber, string word, string sort)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [word],[totaltimes],[lastsearchtime] FROM (SELECT TOP {1} ROW_NUMBER() OVER (ORDER BY SUM([times]) DESC) AS [rowid],[word],SUM([times]) AS [totaltimes],MAX([updatetime]) AS [lastsearchtime] FROM [{0}searchhistories] GROUP BY [word] ORDER BY [totaltimes] DESC) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {1}",
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1);
                else
                    commandText = string.Format("SELECT [word],[totaltimes],[lastsearchtime] FROM (SELECT TOP {1} ROW_NUMBER() OVER (ORDER BY SUM([times]) DESC) AS [rowid],[word],SUM([times]) AS [totaltimes],MAX([updatetime]) AS [lastsearchtime] FROM [{0}searchhistories] WHERE [word] LIKE '%{3}%' GROUP BY [word] ORDER BY [totaltimes] DESC) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {1}",
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1,
                                                word);
        ...
        public int GetSearchWordStatCount(string word)
        {
            ...
            if (string.IsNullOrWhiteSpace(word))
                commandText = string.Format("SELECT COUNT([TBWORD].[CWORD]) FROM (SELECT COUNT([word]) AS CWORD FROM [{0}searchhistories] GROUP BY [word]) AS [TBWORD]", RDBSHelper.RDBSTablePre);
            else
                commandText = string.Format("SELECT COUNT([TBWORD].[CWORD]) FROM (SELECT COUNT([word]) AS CWORD FROM [{0}searchhistories] WHERE [word] LIKE '%{1}%' GROUP BY [word]) AS [TBWORD]", RDBSHelper.RDBSTablePre, word);
            return TypeHelper.ObjectToInt(RDBSHelper.ExecuteScalar(CommandType.Text, commandText));
        }

        #endregion

        #region 获取分类下所有的子分类id（包括自己）
        public StringBuilder GetAllCateIdsByParentCateId(int cateId)
        {           
            ...
            if (cateId > 0)
            {
                commandSelectText = string.Format("with [cte] as ( select [a].[cateid] from [{0}categories] a where [cateid]=@cateId union all select [k].[cateid] from [{0}categories] [k] inner join [cte] [c] on [c].[cateid] = [k].[parentid]  )select * from [cte] ", RDBSHelper.RDBSTablePre);
            }
            else
            {
                commandSelectText = string.Format("select [cateid] from [{0}categories]", RDBSHelper.RDBSTablePre);
            }

            DataTable table = RDBSHelper.ExecuteDataset(CommandType.Text, commandSelectText, parms).Tables[0];

            for (int i = 0; i < table.Rows.Count; i++)
            {
                cateIds.Append(table.Rows[i][0].ToString() + ",");
            }
            cateIds.Remove(cateIds.Length - 1, 1);
            return cateIds;
        }
        #endregion
    }
