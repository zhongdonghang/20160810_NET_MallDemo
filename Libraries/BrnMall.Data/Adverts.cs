...
namespace BrnMall.Data
{
    ...
    public partial class Adverts
    {
        ...
        /// <returns></returns>
        public static string GetAdvertPositionsByPid(int pid)
        {
            string position = "";
            IDataReader reader = BrnMall.Core.BMAData.RDBS.GetAdvertPositionsByPid(pid);
            while (reader.Read())
            {
                position += reader["title"].ToString()+",";
            }
            return position.TrimEnd(',');
        }
