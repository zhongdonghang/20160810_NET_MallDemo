...
namespace BrnMall.Services
{
    ...
    public partial class Carts
    {
        ...
        {
            var sopList = GetCartProductList(-1, sid);
            var deleteList = new List<OrderProductInfo>();
            foreach (var sop in sopList)
            {
                //如果游客购物车里的商品用户购物车里也存在，用户购物车商品count+，删除游客购物车商品
                var uop = Carts.GetCartProductList(uid).FirstOrDefault(i => i.Pid == sop.Pid);
                if (uop != null)
                {
                    List<OrderProductInfo> oplist = new List<OrderProductInfo>();
                    uop.BuyCount += sop.BuyCount;
                    uop.RealCount += sop.RealCount;
                    oplist.Add(uop);
                    Carts.UpdateOrderProductCount(oplist);
                    deleteList.Add(sop);
                }
            ...
            {
                Carts.DeleteOrderProductList(deleteList);
            }
        ...
        public static bool IsOneOrder(string[] selectedCartItemKeyList, List<OrderProductInfo> orderProductList)
        {
            List<OrderProductInfo> seletedList = new List<OrderProductInfo>();
            foreach (var op in orderProductList)
            {
                if (IsSelectCartItem(op.Type, op.Pid, selectedCartItemKeyList))
                {
                    seletedList.Add(op);
                }
            }
            foreach (var item in seletedList)
            {
                if (item.StoreId != seletedList[0].StoreId)
                {
                    return false;
                }
        ...
        /// </summary>
        /// <param name="orderProductList">订单商品列表</param>
        /// <param name="discount">用户id</param>
        /// <returns></returns>
        public static decimal SumOrderProductAmount(List<OrderProductInfo> orderProductList, int uid)
        {
            decimal productAmount = 0M;
            foreach (OrderProductInfo orderProductInfo in orderProductList)
            {
                //商品合计
                decimal Price = Products.GetUserProductPrice(orderProductInfo.Pid, uid);
                productAmount = productAmount + orderProductInfo.BuyCount * Price;
            }
        ...
        /// <param name="discount">用户id</param>
        /// <returns></returns>
        public static decimal SumMallCartOrderProductAmount(List<StoreCartInfo> storeCartList, int uid)
        {
            decimal productAmount = 0M;
            foreach (StoreCartInfo storeCartInfo in storeCartList)
            {
                productAmount += SumOrderProductAmount(storeCartInfo.SelectedOrderProductList, uid);
            }
            return productAmount;
        }
            ...
            if (orderProductInfo.ExtCode4 > 0)
            {
                ...
                if (fullSendMinorOrderProductInfo != null)
                {
                    ...
                    if (fullSendPromotionInfo != null)
                    {
                        ...
                        decimal amount = SumOrderProductAmount(fullSendMainOrderProductList, orderProductInfo.Uid) - orderProductInfo.DiscountPrice * orderProductInfo.BuyCount;
