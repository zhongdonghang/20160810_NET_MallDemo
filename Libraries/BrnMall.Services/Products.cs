...
namespace BrnMall.Services
{
    ...
    public partial class Products
    {
        ...
        {
            List<PartProductInfo> storeTraitProductList = BrnMall.Core.BMACache.Get(string.Format(CacheKeys.MALL_PRODUCT_STORETRAITLIST, storeId, storeCid, trait)) as List<PartProductInfo>;
            if (storeTraitProductList == null)
            {
                ...
                BrnMall.Core.BMACache.Insert(string.Format(CacheKeys.MALL_PRODUCT_STORETRAITLIST, storeId, storeCid, trait), storeTraitProductList);
            }
        ...
        /// <returns></returns>
        public static decimal GetUserProductPrice(int pid, int uid)
        {
            PartProductInfo product = GetPartProductById(pid);
            ...
            UserRankInfo rank = UserRanks.GetUserRankById(user.UserRid);
            if (rank == null || rank.System != 0)
            {
                return product.ShopPrice;
            }
            //普通会员,会员价为原价
            if (rank.CreditsLower == 0)
            {
                return product.ShopPrice;
            }
            //超级会员，享受队员价
            if (rank.CreditsUpper == -1)
            {
            ...
            {
                decimal price = decimal.Round(product.ShopPrice * rank.Discount / 10, 2);
                return price;
            }
        }
    }
