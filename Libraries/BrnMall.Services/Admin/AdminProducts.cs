...
namespace BrnMall.Services
{
    ...
    public partial class AdminProducts : Products
    {
        ...
        public static int AddProduct(ProductInfo productInfo, int stockNumber, int stockLimit, List<ProductAttributeInfo> productAttributeList)
        {
            ...
            //店铺推荐设置
            if (productInfo.IsBest == 1)
            {
                BMACache.Remove(string.Format(CacheKeys.MALL_PRODUCT_STORETRAITLIST, productInfo.StoreId, 0, 0));
            }
            //店铺热销设置
            if (productInfo.IsHot == 1)
            {
                BMACache.Remove(string.Format(CacheKeys.MALL_PRODUCT_STORETRAITLIST, productInfo.StoreId, 0, 1));
            }
            //店铺新品设置
            if (productInfo.IsNew == 1)
            {
                BMACache.Remove(string.Format(CacheKeys.MALL_PRODUCT_STORETRAITLIST, productInfo.StoreId, 0, 2));
            }
            
            //商品类别关联的品牌缓存修改
            List<BrandInfo> brandList = BrnMall.Core.BMACache.Get(CacheKeys.MALL_CATEGORY_BRANDLIST + productInfo.CateId) as List<BrandInfo>;
            BrandInfo binfo = Brands.GetBrandById(productInfo.BrandId);
            if (binfo != null && brandList != null && !brandList.Contains(binfo))
            {
                BrnMall.Core.BMACache.Remove(CacheKeys.MALL_CATEGORY_BRANDLIST + productInfo.CateId);
                BrnMall.Core.BMACache.Remove(CacheKeys.MALL_CATEGORY_BRANDLIST + 0);
            }
        ...
        {
            var oldproduct = GetProductById(productInfo.Pid);
            UpdateProduct(productInfo);
            UpdateProductStock(productInfo.Pid, stockNumber, stockLimit);

            if (oldproduct.IsBest != productInfo.IsBest)
            {
                BMACache.Remove(string.Format(CacheKeys.MALL_PRODUCT_STORETRAITLIST, productInfo.StoreId, 0, 0));
            }
            if (oldproduct.IsHot != productInfo.IsHot)
            {
            BMACache.Remove(string.Format(CacheKeys.MALL_PRODUCT_STORETRAITLIST, productInfo.StoreId, 0, 1));
            }
            if (oldproduct.IsNew != productInfo.IsNew)
            {
                BMACache.Remove(string.Format(CacheKeys.MALL_PRODUCT_STORETRAITLIST, productInfo.StoreId, 0, 2));
            }
        ...
        /// <param name="storeId">店铺id</param>
        public static bool ChangeProductIsNew(int[] pidList, int isNew,int storeId)
        {
            if (pidList != null && pidList.Length > 0)
            {
                BrnMall.Core.BMACache.Remove(string.Format(CacheKeys.MALL_PRODUCT_STORETRAITLIST, storeId, 0, 2));
        ...
        /// <param name="storeId">店铺id</param>
        public static bool ChangeProductIsHot(int[] pidList, int isHot,int storeId)
        {
            if (pidList != null && pidList.Length > 0)
            {
                BrnMall.Core.BMACache.Remove(string.Format(CacheKeys.MALL_PRODUCT_STORETRAITLIST, storeId, 0, 1));
        ...
        /// <param name="storeId">店铺id</param>
        public static bool ChangeProductIsBest(int[] pidList, int isBest,int storeId)
        {
            if (pidList != null && pidList.Length > 0)
            {
                BrnMall.Core.BMACache.Remove(string.Format(CacheKeys.MALL_PRODUCT_STORETRAITLIST, storeId, 0, 0));
        ...
        /// <param name="pid">商品id</param>
        /// <param name="shopPrice">商城价格</param>
        public static bool UpdateProductVipPrice(int pid, decimal vipPrice)
        {
            if (pid < 1 || vipPrice < 0M) return false;
            return BrnMall.Data.Products.UpdateProductVipPrice(pid, vipPrice);
