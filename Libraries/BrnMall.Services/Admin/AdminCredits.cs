...
namespace BrnMall.Services
{
    ...
    public partial class AdminCredits : Credits
    {
        ...
        public static void AdminSendCredits(PartUserInfo partUserInfo, int rankCredits, int sendUid, DateTime sendTime)
        {
        ...
        /// <param name="sendUid">发放用户id</param>
        /// <param name="sendTime">发放时间</param>
        public static void WithdrawalPass(int wid)
        {
            var winfo = Credits.GetWithdrawalLogById(wid);
            if (winfo == null)
            {
                return;
            }
            ...
            CreditLogInfo cinfo = new CreditLogInfo()
            {
                Uid = winfo.Uid,
                RankCredits = 0,
                Action = (int)CreditAction.WithdrawCash,
                ActionCode = winfo.RecordId,
                ActionDes = "您提现了" + winfo.ApplyAmount + "元",
                Operator = winfo.OperatorUid,
                FrozenAmount = 0,
                UserAmount = -winfo.ApplyAmount,
